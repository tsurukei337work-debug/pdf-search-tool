...
class ConfigManager:
    ...
    def load_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return dict(self.config['Settings']) if 'Settings' in self.config else self.DEFAULT_CONFIG['Settings'].copy()
        except:
            return self.DEFAULT_CONFIG['Settings'].copy()
...
class PDFSearchApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("PDF文字列検索ツール (PyMuPDF 高速版)")
    ...
    def _process_search(self, folder, keyword):
        try:
            ...
            self.var_status.set(f"高速検索開始: {total}件...")
            processed_count = 0
            
            # 並列処理 (PyMuPDFはCPUも使うため、ワーカー数はCPUコア数依存が良いが、ここではバランス型で5)
